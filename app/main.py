...
Fetches real-time exchange rates from CoinGecko API and displays them in a presentable web interface.
"""
...
# Prometheus metrics
REQUEST_COUNT = Counter('crypto_app_requests_total', 'Total app requests', ['method', 'endpoint'])
REQUEST_LATENCY = Histogram('crypto_app_request_duration_seconds', 'Request latency')
API_CALLS = Counter('coingecko_api_calls_total', 'Total CoinGecko API calls', ['status'])
...
class CryptoRatesAPI:
    ...
    def get_exchange_rates(self):
        ...
        try:
            ...
            logger.info(f"Successfully fetched {len(self.cached_data['rates'])} exchange rates")
