...
class TestCryptoRatesAPI:
    ...
    def test_successful_api_call(self, m):
    ...
    def test_api_timeout(self, m):
        ...
        assert "timeout" in result["error"].lower() or "unable to fetch" in result["error"].lower()
    
    @requests_mock.Mocker()
    def test_api_error_response(self, m):
    ...
    def test_caching_mechanism(self):
        ...
        with patch.object(self.api, '_fetch_fresh_data') as mock_fetch:
...
class TestFlaskApp:
    ...
    def test_index_route_api_error(self, mock_get_rates):
        ...
        assert b"error" in response.data.lower() or b"something went wrong" in response.data.lower()
